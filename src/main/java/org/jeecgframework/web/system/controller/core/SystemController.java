...
//    }
	...
	public AjaxJson saveDepart(TSDepart depart, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(depart.getId())) {
			TSDepart depart2 = systemService.getEntity(TSDepart.class, depart.getId());
			UpdateUtils.copyNonNullProperties(depart, depart2);
			userService.saveOrUpdate(depart2);
			//userService.executeSql("update t_s_depart set province='11' where id =?", depart.getId());
...
//			depart.setOrgCode(orgCode);
	 ...
	 * @return
	 */
	@RequestMapping(params = "saveDepartPoint")
	@ResponseBody
	/*public AjaxJson saveDepartPoint(TSDepart depart, HttpServletRequest request) {
		String message = null;
	    //不能自己给自己授权积分 只能授权下级积分  下级积分从上级减扣
		/*TSUser user = ResourceUtil.getSessionUser();
		logger.info("++1+"+user.getDepartid());
		TSDepart TSPDepart = systemService.getEntity(TSDepart.class, depart.getTSPDepart().getId());
		logger.info("===TSPDepart=="+JSON.toJSONString(TSPDepart));
		AjaxJson j = new AjaxJson();
		if(depart.getId().equals(user.getDepartid())){
			j.setMsg("不能授权本组织积分");
			return j;
		}
		if (StringUtil.isNotEmpty(depart.getId())) {
			if(StringUtil.isEmpty(depart.getSqpoint())){
				depart.setSqpoint("0");
			}
			if(StringUtil.isEmpty(depart.getSqpointadd())){
				depart.setSqpointadd("0");
			}
			if(StringUtil.isEmpty(depart.getSqpointused())){
				depart.setSqpointused("0");
			}
			int point =Integer.parseInt(depart.getSqpoint()) ;
			int pointused =Integer.parseInt(depart.getSqpointused()) ;
			int havepoint=point-pointused;//当前组织剩余积分
			logger.info("===havepoint=="+havepoint);
			int chgpoint =Integer.parseInt(depart.getSqpointadd()) ;//本次调整积分
			logger.info("===havepoin3333t=="+JSON.toJSONString(depart.getTSPDepart()));
			int pusedpoint =Integer.parseInt(TSPDepart.getSqpointused()) ;//上级已经用的积分
			int ppoint =Integer.parseInt(TSPDepart.getSqpoint()) ;//上级已经用的积分
			int phavepoint=ppoint-pusedpoint;//父亲组织剩余积分
			logger.info("===phavepoint=="+phavepoint);
			String orgcode=depart.getOrgCode();
			
			if("addpoint".equalsIgnoreCase(depart.getOperflag())){
				if(phavepoint>chgpoint){
					//上级扣减
					TSDepart depart3 = systemService.getEntity(TSDepart.class, depart.getTSPDepart().getId());
					depart3.setSqpointused(""+(pusedpoint+chgpoint));
					//UpdateUtils.copyNonNullProperties(depart, depart3);
					userService.saveOrUpdate(depart3);
					 //下级增加积分   
					TSDepart depart2 = systemService.getEntity(TSDepart.class, depart.getId());
					//UpdateUtils.copyNonNullProperties(depart, depart2);
					depart2.setSqpoint(""+(point+chgpoint));
					userService.saveOrUpdate(depart2);
					message="操作成功";
			...
			if("delpoint".equalsIgnoreCase(depart.getOperflag())){
				if(havepoint>chgpoint){
					//上级加积分
					TSDepart depart3 = systemService.getEntity(TSDepart.class, depart.getTSPDepart().getId());
					depart3.setSqpointused(""+(pusedpoint-chgpoint));
					//UpdateUtils.copyNonNullProperties(depart, depart3);
