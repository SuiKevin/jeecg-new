...
//				}
				...
				//新增用户补充一张积分表
				systemService.executeSql("insert into qp_jifen(id,point_day_limit) values(?,300)", user.getId());
				systemService.executeSql("insert into qp_jobtype(id) values(?)", user.getId());
	...
	@RequestMapping(params = "bindidcard")
	public ModelAndView bindIdCard(TSUser user, HttpServletRequest req) {
        TSDepart tsDepart = new TSDepart();
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());			
			req.setAttribute("user", user);
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("system/user/bindidcard");
	}
