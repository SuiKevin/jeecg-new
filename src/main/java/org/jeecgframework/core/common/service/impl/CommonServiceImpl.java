...
public class CommonServiceImpl implements CommonService {
	 ...
	 * @return
	 */
	public <T> DataGrid findDataGridForJdbc(DataGrid dataGrid,String sql,Class<T> clazz) {
		//查询工单总数
		List<Map<String, Object>> list = commonDao.findForJdbc(sql,null);
		if(list.size() >0 ){
			List<T> resultList = commonDao.findObjForJdbc(sql, dataGrid.getPage(), dataGrid.getRows(), clazz);
			dataGrid.setResults(resultList);
			dataGrid.setTotal(list.size());
		}else{
			dataGrid.setTotal(0);
			dataGrid.setResults(new ArrayList<Map<String, Object>>());
		}
		return dataGrid;
	}
